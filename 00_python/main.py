...
# hey!!! we can nest try-except blocks!!!!!!!
...
# CHAPTER 10: TEST YOUR CODE

# well, all of that tdd ideology

# lets see how this works using a simple function as exemple

def get_formatted_name(first, last):
    full_name = f"{first} {last}"
    return full_name.title()

# this is a simple function that for now works correctly, but by writing a test, we ensure that if we need to modify it in a later time, it will continue to work

def test_get_formatted_name():
    formatted_name = get_formatted_name("joão", "silva")
    assert formatted_name == "João Silva"

# lets take a look on this function: in order to pytest to work, we need to start its name with "test_"
# second, we make an assertation, that declares what the output of the function should be
# to test it, just run pytest <your_file.py>

# if you create a new parameter for this function for example, pytest will report a error. 
# then you can react to it in many ways, as making the third parameter optional or rewriting the test
# anyways, this is just a simple test, but you can use pytest and asserts in many ways in your program

# you can do assertations in many ways, using ==, !=, a (boolean), not a (again boolean, not in python operate as !), 
# "assert element in list", "assert element not in list"...

# test a class:

# lets create a class so we can test it
...
# this is the object
question = "What are your favorite records?"
record_survey = anon_survey(question)

## this is how we collect data 
#print("Enter 'q' anytime to quit.")
#while True:
#    response = input("Record: ")
#    if response == 'q':
#        break;
#    record_survey.store_response(response)

# this is how we show the results
print("Here are the results: ")
record_survey.show_results()

# now we are going to write tests for it
def test_anon_survey():
    question = "What are your favorite records?"
    record_survey = anon_survey(question)
    responses = ['graduation', 'dark fantasy', 'donda']
    for response in responses:
        record_survey.store_response(response)
    for response in responses:
        assert response in record_survey.responses

# obs: if your program requires interaction, you will need to interact. use -s to do it.
# obs: when writing tests, the design you need to have in mind is: having the input, is the output correct? 

# FIXTURES

# if we test anon_survey for one or three inputs. or... in general, if we need to write multiple tests for a class, we can do it manually, which is fine but too much work
# in tests, a fixture helps creating a test environment, and this often means creating a resource that's used for more than one test

# we create a fixture in python by writing a function with the decoretor @pytest.fixture
...
def records_survey():
    question = "What are your favorite records?"
    records_survey = anon_survey(question)
    return records_survey

def test_records_survey_one_answer(records_survey):
    records_survey.store_response("dark fantasy")
    assert "dark fantasy" in records_survey.responses

def test_records_survey_three_answer(records_survey):
    responses = ['graduation', 'dark fantasy', 'donda']
    for response in responses:
        record_survey.store_response(response)
    for response in responses:
...
# so... basically fixtures allows you to create a basic block of a function that you can reuse it in other functions. afaict, only for pytest
# this is kinda redundant with parent class/child class so i think this is just for pytest and similar libs
